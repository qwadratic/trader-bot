...
w3 = Web3(HTTPProvider(config.ETH_NODE))
...
def create_usdt_tx(from_address, to_address, value, private_key, gasPrice):
    ...
    token = w3.eth.contract(Web3.toChecksumAddress(USDT_CONTRACT_ADDRESS), abi=usdtABI)
...
def get_balance(address, currency):
    ...
    if currency == 'USDT':
        usdt_erc20 = w3.eth.contract(Web3.toChecksumAddress(USDT_CONTRACT_ADDRESS), abi=usdtABI)
